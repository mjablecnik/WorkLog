...
/**
 * A block column taller than this takes `.tl-h-fill` instead of a ladder class — also
 * the top of the `.tl-h-*` ladder generated by `scripts/generate-palette-and-heights.ts`,
 * which imports this constant so the ladder's ceiling and this threshold can never drift
 * apart.
 */
export const FILL_THRESHOLD_PX = 320;
...
export function layOutDay(
	...
	for (const rendered of sizeableByBlock) {
		...
		if (tallestIndex >= 0 && remainder > 0) {
			// The remainder is itself a fraction of a pixel-ladder step. It must be
			// FLOORED here, never rounded: Math.round can round up, and handing back
			// more than the true remainder can push the block's total a pixel past its
			// pre-quantisation share — which, summed across several single-segment
			// blocks, can push the whole layout's total past availablePx even when the
			// budget comfortably allows it (caught by Property 2's property test).
			// Flooring can only give back less than the true remainder, so the block's
			// total can undershoot its exact pre-quantisation value by under a pixel,
